...
export async function uploadFiles(
  ...
  const response = await fetch(
    `${getGatewayBaseURL()}/api/threads/${threadId}/uploads`,
    {
      method: "POST",
      body: formData,
    },
...
export async function listUploadedFiles(
  ...
  const response = await fetch(
    `${getGatewayBaseURL()}/api/threads/${threadId}/uploads/list`,
  );
...
export async function deleteUploadedFile(
  ...
  const response = await fetch(
    `${getGatewayBaseURL()}/api/threads/${threadId}/uploads/${filename}`,
    {
      method: "DELETE",
    },
  );
